...
import { Search, MapPin, Users, Edit2, Trash2, Plus, LayoutGrid, LogIn, CheckCircle2, Clock } from 'lucide-react';
...
const FacilitiesDashboard = () => {
  ...
  const fetchResources = async () => {
    ...
    try {
      ...
      const url = hasFilters ? `/api/resources/search?${params.toString()}` : '/api/resources';
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Are you sure you want to decommission this asset? This action is permanent.')) return;
    try {
            ...
            <div key={res.id} className="bg-white rounded-[3rem] overflow-hidden shadow-xl hover:shadow-[0_32px_64px_-16px_rgba(0,0,0,0.1)] transition-all duration-500 border border-slate-100 group flex flex-col relative transform hover:-translate-y-2">
              ...
              <div className="relative h-64 overflow-hidden">
                ...
                {currentUser?.role === 'ADMIN' && (
                  <div className="absolute top-6 right-6 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => openEditModal(res)} className="p-3 bg-white/90 backdrop-blur text-sliit-blue rounded-2xl hover:bg-white shadow-xl transition-all">
                      <Edit2 size={18} />
                    </button>
                    <button onClick={() => handleDelete(res.id)} className="p-3 bg-white/90 backdrop-blur text-rose-500 rounded-2xl hover:bg-white shadow-xl transition-all">
                      <Trash2 size={18} />
                    </button>
                  </div>
                )}
              ...
              <div className="p-10 flex-1 flex flex-col">
                ...
                <h3 className="text-2xl font-black text-slate-800 mb-6 group-hover:text-sliit-blue transition-colors leading-tight">
                  <Link to={`/resources/${res.id}`} className="hover:underline focus:outline-none focus:ring-2 focus:ring-sliit-orange rounded">
                    {res.name}
                  </Link>
                </h3>
