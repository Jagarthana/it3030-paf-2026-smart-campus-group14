...
const BookingsDashboard = () => {
  ...
  const fetchBookings = async () => {
    setLoading(true);
    try {
      const endpoint = currentUser?.role === 'ADMIN' ? '/api/bookings' : '/api/bookings/my';
  ...
  const handleApprove = async (id) => {
    ...
    try {
      ...
      alert(typeof msg === 'string' ? msg : 'Approve failed. Another booking may already be approved for this time.');
    } finally {
  ...
  const handleReject = async (id) => {
    ...
    setProcessingId(id);
    try {
      await axios.put(`/api/bookings/${id}/reject`, { reason: reason || '' });
      fetchBookings();
    } catch (error) {
      const msg = error.response?.data;
      alert(typeof msg === 'string' ? msg : 'Reject failed.');
    } finally {
      setProcessingId(null);
    }
  };
  ...
  const handleCancel = async (id) => {
    ...
    setProcessingId(id);
    try {
      await axios.delete(`/api/bookings/${id}/cancel`);
      fetchBookings();
    } catch (error) {
        ...
        <div>
          ...
          <p className="text-slate-500 mt-2 font-medium">
            {currentUser?.role === 'ADMIN' 
              ? 'Review, approve, or reject facility booking requests. Use filters to narrow by status.' 
              : 'Track pending requests and your booking history.'}
          </p>
        </div>
