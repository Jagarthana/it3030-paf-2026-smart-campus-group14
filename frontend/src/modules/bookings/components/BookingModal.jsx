...
const BookingModal = ({ isOpen, onClose, resource, onBookingSuccess }) => {
  ...
  const computeRange = () => {
    ...
    const endHour = parseInt(formData.startTime.split(':')[0], 10) + parseInt(formData.duration, 10);
  ...
  useEffect(() => {
    if (!isOpen || !resource?.id || !formData.date || !formData.startTime) {
      setConflictWarning(false);
      return;
    }
    const timer = setTimeout(async () => {
      try {
        ...
        const res = await axios.get(`/api/bookings/conflict-check?${params.toString()}`);
        setConflictWarning(res.data?.conflict === true);
      } catch {
        setConflictWarning(false);
      }
    }, 450);
    return () => clearTimeout(timer);
  }, [isOpen, resource, formData.date, formData.startTime, formData.duration]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { startDateTime, endDateTime } = computeRange();
      ...
      const msg =
        typeof raw === 'string'
          ? raw
          : raw?.message || 'Failed to submit booking request. Please check availability windows.';
      setError(msg);
    } finally {
      setLoading(false);
    }
    ...
    <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md transition-all animate-in fade-in duration-300">
      <div className="bg-white rounded-[3rem] w-full max-w-xl shadow-2xl overflow-hidden border border-slate-100 animate-in zoom-in-95 duration-300">
        ...
        <form onSubmit={handleSubmit} className="p-10 space-y-8">
          ...
          {conflictWarning && (
            <div className="bg-amber-50 text-amber-900 p-5 rounded-3xl text-sm font-bold flex items-center gap-4 border border-amber-200 animate-in slide-in-from-top-2">
              <AlertCircle size={22} className="shrink-0 text-amber-600" />
              <span>
                This slot overlaps an <strong className="font-black">approved</strong> booking for this resource. Choose another time or you will not be able to submit.
              </span>
            </div>
          )}
