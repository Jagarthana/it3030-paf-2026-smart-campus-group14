...
public interface BookingRepository extends JpaRepository<Booking, Long> {
    ...
    /** Overlap with any APPROVED booking (used when creating a new request — no double booking). */
    @Query("SELECT COUNT(b) FROM Booking b WHERE b.resource.id = :resourceId " +
           "AND b.status = 'APPROVED' " +
           "AND ((b.startTime < :endTime AND b.endTime > :startTime))")
    long countApprovedOverlappingBookings(@Param("resourceId") Long resourceId,
